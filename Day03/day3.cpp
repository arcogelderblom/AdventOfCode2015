...
int main(void) 
{
    ...
    std::cout << "Day 3, puzzle 1: " << std::flush << deliverPresents(instructions[0]).size() << std::endl;
    auto t_end = std::chrono::high_resolution_clock::now();
    std::cout << "Completed in: " << std::chrono::duration<double, std::milli>(t_end - t_begin).count() << " ms" << std::endl;

    t_begin = std::chrono::high_resolution_clock::now();
    std::cout << "Day 3, puzzle 2: " << std::flush << deliverPresentsWithRoboSanta(instructions[0]).size() << std::endl;
