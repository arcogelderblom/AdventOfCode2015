...
void executeAndSortInstructions(std::vector<std::string>& instructions, std::map<std::string, unsigned short>& wires)
{
    ...
    while (instructions.size() > 0)
    {
        ...
        {
            std::vector<std::string> instruction_elements = StringOperations::splitString(instructions[i], " ");
...
int main(void) 
{
    InputFile input_file("InputFiles/day07.txt");
    std::vector<std::string> input = input_file.getContentAsString("\n");
    ...
    for (; i < input.size(); i++)
    {
        if (*(StringOperations::splitString(input[i], " ").rbegin()) == "b")
        {
            break;
        }
    }
