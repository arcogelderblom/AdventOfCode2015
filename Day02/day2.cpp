...
{
    std::vector<std::string> separatedDimensions = StringOperations::splitString(dimensions, "x");
    // 2*l*w + 2*w*h + 2*h*l
...
int main(void) 
{
    ...
    std::cout << "Day 2, puzzle 1: " << std::flush << getSquareFeetWrappingPaper(instructions) << std::endl;
    auto t_end = std::chrono::high_resolution_clock::now();
    std::cout << "Completed in: " << std::chrono::duration<double, std::milli>(t_end - t_begin).count() << " ms" << std::endl;

    t_begin = std::chrono::high_resolution_clock::now();
    std::cout << "Day 2, puzzle 2: " << std::flush << getFeetOfRibbon(instructions) << std::endl;
